...
values ('CUST-001', 'Mario Rossi'),
       ('CUST-002', 'Giulia Bianchi'),
       ('CUST-003', 'Luca Verdi') on conflict (customer_code) do nothing;

insert into products (product_code, name)
values ('HOME-BASIC', 'Home Basic'),
       ('HOME-PLUS', 'Home Plus'),
       ('TRAVEL-BASIC', 'Travel Basic') on conflict (product_code) do nothing;
...
values ('HOME-FIRE',
       ...
       ('TRAVEL-BAGGAGE',
        ...
        1.10,
        (select id from products where product_code = 'TRAVEL-BASIC')) on conflict (code) do nothing;
...
values ('POL-2026-001',
       ...
       ('POL-2026-002',
        ...
        date '2026-06-01',
        date '2026-06-15',
        'ACTIVE') on conflict (policy_number) do nothing;

insert into policy_coverage (policy_id, coverage_id)
values ((select id from policies where policy_number = 'POL-2026-001'),
       ...
       ((select id from policies where policy_number = 'POL-2026-002'),
        (select id from coverages where code = 'TRAVEL-BAGGAGE')) on conflict (policy_id, coverage_id) do nothing;

insert into premiums (policy_id, amount, due_date)
select policy_id, amount, due_date
from (values ((select id from policies where policy_number = 'POL-2026-001'),
              365.00,
              date '2026-01-01'),
             ((select id from policies where policy_number = 'POL-2026-002'),
              57.40,
              date '2026-06-01')) as seed(policy_id, amount, due_date)
...
values ('PAY-2026-001',
        ...
        365.00,
        date '2026-01-02',
        'PAID') on conflict (external_reference) do nothing;
...
values ('CLM-2026-001',
        ...
        date '2026-03-11',
        450.00,
        'OPENED') on conflict (claim_number) do nothing;
